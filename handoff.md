...
2. 汇总 EXP-01 在武汉验证 patch 上的指标；不再额外执行完整 Wuhan 场景推理；
...
#### EXP-01 — City-disjoint patch validation

- **Protocol:** Train each model on the Beijing patches and evaluate it on the Wuhan validation patches generated by `train_B_val_W`. Use the existing validation loop to report global and per-class mIoU, macro-F1, OA/ACC, and Kappa from the confusion matrix.
- **Required record:** Train/validation scene names, split-file paths, patch size, number of patches, and whether Wuhan is used for model selection.
- **Caution:** The split is geographically disjoint between training and validation scenes, but Wuhan is still a validation/model-selection scene. Report it as city-disjoint validation, not as a completely held-out test set.
- **Deliverables:** City-split checkpoints, validation metric table, per-class metrics where available, and a short split/provenance note. Full-scene Wuhan inference is not required.
